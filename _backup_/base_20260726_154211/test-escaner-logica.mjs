...
// El caso que destapó todo: "BOTE LG" con ruido de empaque. El fuzzy viejo
// llegaba al Bote Light con duda razonable — estaba equivocado: el ticket
// IMPRIME 940 ml y el tamaño impreso manda sobre la palabra "BOTE", así que
// el diccionario universal lo resuelve a la Ballena Light (igual a2). Si en
// la tienda ese código es otra cosa, el alias confirmado manda (ver a1).
const e1 = E.emparejarProducto(
  "BOTE LG",
  "Pacífico LG 12 940 ml CT Caja Plástica",
  cervezas
);
assert.equal(e1.producto?.nombre, "Ballena Pacífico Light", `salió ${e1.producto?.nombre} (${e1.estado} ${e1.score})`);
assert.equal(e1.estado, "seguro", "igualdad con el canónico 940 → seguro");
assert.equal(e1.viaDiccionario, true, "lo resolvió el diccionario universal");
assert.ok(!e1.notaConflicto, "940 ballena vs Ballena (sin dígitos): sin conflicto");
...
// --- tamaño canónico y envase: tamanoCanonicaEnvase (TAREA 1a) ----------------
// Orden de confianza: ml explícitos → litros (incluye L pelada) → empaque
// pegado → tamaño pelado → palabra de envase SOLO sin dígitos. 1000 → 940.
assert.deepEqual(D.tamanoCanonicaEnvase("MODELO ESP 12 PK 1L CT"), { ml: 940, envase: "ballena" }, "1L pelada → ballena 940");
assert.deepEqual(D.tamanoCanonicaEnvase("PACIFICO 940"), { ml: 940, envase: "ballena" }, "940 pelado");
assert.deepEqual(D.tamanoCanonicaEnvase("TECATE 1.2L"), { ml: 1200, envase: "mega" }, "1.2L → mega");
assert.deepEqual(D.tamanoCanonicaEnvase("VICTORIA 473 ML"), { ml: 473, envase: "latón" }, "473 → latón");
assert.deepEqual(D.tamanoCanonicaEnvase("BOTE 355"), { ml: 355, envase: "bote" }, "355 → bote");
assert.deepEqual(D.tamanoCanonicaEnvase("CUAR190"), { ml: 190, envase: "cuartito" }, "CUAR pegado → cuartito");
assert.equal(D.tamanoCanonicaEnvase("COCA 600ML"), null, "600 no es tamaño cervecero → null");
assert.deepEqual(D.tamanoCanonicaEnvase("BALLENA SUAVE"), { ml: 940, envase: "ballena" }, "palabra ballena SIN dígitos");
console.log("✓ tamanoCanonicaEnvase (8 casos)");

// --- tamaño impreso manda + variantes conservadas (TAREA 1b, casos del brief) --
const p1d = D.buscarEnDiccionario("MODELO ESP 12 PK 1L CT", null);
assert.equal(p1d?.nombre, "Ballena Modelo Especial 940 ml", "1L CT → ballena 940");
assert.equal(p1d?.presentacion, "940 ml");
assert.equal(p1d?.piezasEmpaque, 12, "ballena: caja 12");
assert.equal(p1d?.tamanoMl, 940);
assert.equal(p1d?.envase, "ballena");

const p2d = D.buscarEnDiccionario("VICTORIA LIMON Y SAL BT 4PK H C 24 473 ML", null);
assert.equal(p2d?.nombre, "Latón Victoria Limón y Sal 473 ml", "sabores del crudo se conservan");
assert.equal(p2d?.presentacion, "473 ml");
assert.equal(p2d?.piezasEmpaque, 24, "latón: caja 24");
assert.equal(p2d?.tamanoMl, 473);
assert.equal(p2d?.envase, "latón");

// 473 gana aunque el crudo diga "BOTE"; "regular" equivale a "clara".
const p3d = D.buscarEnDiccionario("PACIFICO REGULAR BOTE 4PKH C 24 473 ML", null);
assert.equal(p3d?.nombre, "Latón Pacífico Clara 473 ml", "el tamaño impreso manda sobre 'BOTE'; regular=clara");
assert.equal(p3d?.presentacion, "473 ml");
assert.equal(p3d?.piezasEmpaque, 24);

const p4d = D.buscarEnDiccionario("ballena pacifico suave 940", null);
assert.equal(p4d?.nombre, "Ballena Pacífico Suave 940 ml", "ballena suave 940");
assert.equal(p4d?.piezasEmpaque, 12);
assert.equal(p4d?.tamanoMl, 940);
assert.equal(p4d?.envase, "ballena");
console.log("✓ tamaño impreso manda + variantes (4 casos del brief)");

// --- conflictoPresentacion: nota humana, sin choque y exenciones (TAREA 1d) ---
const conf1 = D.conflictoPresentacion("MODELO ESP 12 PK 1L CT", "Bote Modelo Especial 355 ml");
assert.ok(typeof conf1 === "string", "940 vs 355 → conflicto");
assert.ok(conf1.startsWith("la presentación no coincide"), `mensaje: ${conf1}`);
assert.ok(conf1.includes("1 L"), `etiqueta cruda del ticket: ${conf1}`);
assert.ok(conf1.includes("355 ml"), `etiqueta del producto: ${conf1}`);
assert.equal(
  D.conflictoPresentacion("PACIFICO 473 ML", "Latón Pacífico 473 ml"),
  null,
  "mismo tamaño y envase → sin conflicto"
);
assert.equal(
  D.conflictoPresentacion("PACIFICO LG 940", "Ballena Pacífico Light"),
  null,
  "palabra ballena sin dígitos en el producto → mismo envase → sin conflicto"
);
console.log("✓ conflictoPresentacion (3 casos)");

// --- flujo completo: el conflicto DEGRADA el empareje, no lo bloquea ----------
const prodsSuave = [
  { id: "bote-suave", nombre: "Bote Pacífico Suave 410ml", codigo_barras: null, costo_centavos: null, precio_venta_centavos: 0 },
];
const mSuave = E.emparejarLineas(
  [{ ...base, nombre_crudo: "ballena pacifico suave 940", nombre_sugerido: "ballena pacifico suave 940" }],
  prodsSuave
);
assert.equal(mSuave[0]?.producto?.id, "bote-suave", "mismo producto, otra presentación: empareja");
assert.equal(mSuave[0]?.estado, "sugerido", "conflicto de presentación → JAMÁS seguro");
assert.ok(typeof mSuave[0]?.notaConflicto === "string", "lleva nota humana");
assert.ok(mSuave[0].notaConflicto.includes("940 ml") && mSuave[0].notaConflicto.includes("410 ml"), `nota: ${mSuave[0].notaConflicto}`);
assert.equal(mSuave[0]?.viaDiccionario, true, "la ficha viaja adjunta");

// EXCEPCIÓN 1: el alias confirmado MANDA aunque la presentación choque.
const aliasOtroTam = new Map([["light pacifico", { productoId: "bote", piezasEmpaque: null }]]);
const aExc = E.emparejarProducto("PACIFICO LG 940", "Pacífico Light 940 ml", cervezas, aliasOtroTam);
assert.equal(aExc.producto?.id, "bote", "el alias apunta al bote pese al 940 del ticket");
assert.equal(aExc.estado, "seguro", "alias confirmado → seguro");
assert.equal(aExc.viaAlias, true);
assert.ok(aExc.notaConflicto == null, "el alias está EXENTO del conflicto");

// EXCEPCIÓN 2: el barcode exacto también está exento (el EAN es presentación-específica).
const prodsBc = [
  { id: "laton-vic", nombre: "Latón Victoria 473 ml", codigo_barras: "75012345", costo_centavos: null, precio_venta_centavos: 0 },
];
const mBc = E.emparejarLineas(
  [{ ...base, nombre_crudo: "VICTORIA 940", nombre_sugerido: "VICTORIA 940", codigo_barras: "75012345" }],
  prodsBc
);
assert.equal(mBc[0]?.producto?.id, "laton-vic", "barcode exacto gana");
assert.equal(mBc[0]?.estado, "seguro");
assert.ok(mBc[0]?.notaConflicto == null, "barcode exacto EXENTO del conflicto");
console.log("✓ conflicto degrada a sugerido + exenciones (alias y barcode)");
