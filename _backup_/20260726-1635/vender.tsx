    ...
    <SafeAreaView style={est.raiz} edges={["top"]}>
      ...
      {cobroAbierto && (
        <ModalCobro
          ...
          onCobrado={async (r, metodo, pagadoCentavos) => {
            ...
            const t = await construirTicket(r, carrito, metodo, pagadoCentavos);
            setCarrito([]);
            setTicketCobrado(t);
            cargar();
          }}
...
function ModalCobro({
  ...
  items: ItemCarrito[];
  onCerrar: () => void;
  onCobrado: (r: ResultadoCobro, metodo: MetodoPago, pagadoCentavos: number) => void;
}) {
  const { tema: T } = useTema();
  const est = useMemo(() => crearEstilos(T), [T]);
  const estiloInput = useEstiloInput();
  ...
  async function confirmar() {
    ...
    try {
      ...
      onCobrado(r, metodo, metodo === "efectivo" ? recibidoC : total);
    } catch (e: any) {
      setError(e?.message ?? String(e));
      setCobrando(false);
    }
