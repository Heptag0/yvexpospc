...
// v3.1 — espejo EXACTO de tienda_utils.py:
// TEMAS_VALIDOS = auto, perla, medianoche, arena, cielo.
// Alias viejos: claro -> perla, oscuro -> medianoche.
export type TemaTienda = "auto" | "perla" | "medianoche" | "arena" | "cielo";
...
export const TEMA_AUTO_CLARO: TemaTienda = "perla";
export const TEMA_AUTO_OSCURO: TemaTienda = "medianoche";

/** Canonicaliza el tema de la tienda. Espejo de tienda_utils.validar_tema:
 *  acepta los alias viejos y cualquier valor raro cae a "auto". */
export function temaTiendaValido(tema: string | null | undefined): TemaTienda {
  let t = (typeof tema === "string" ? tema : "").trim().toLowerCase();
  t = ALIAS_TEMAS[t] ?? t;
  return (TEMAS_TIENDA as string[]).includes(t) ? (t as TemaTienda) : "auto";
}

/** Resuelve "auto" al tema concreto según el sistema (oscuro -> medianoche).
 *  Los demás temas pasan intactos. */
export function temaTiendaEfectivo(
  tema: string | null | undefined,
  sistemaOscuro: boolean
): TemaTienda {
  const t = temaTiendaValido(tema);
  if (t !== "auto") return t;
  return sistemaOscuro ? TEMA_AUTO_OSCURO : TEMA_AUTO_CLARO;
}
...
/** Fondo guía del tema (resolviendo auto con el sistema), para el aviso
 *  de contraste al elegir el acento. */
export function fondoGuiaTema(
  tema: string | null | undefined,
  sistemaOscuro: boolean
): string {
  return FONDOS_TEMA[temaTiendaEfectivo(tema, sistemaOscuro) as Exclude<TemaTienda, "auto">];
}
