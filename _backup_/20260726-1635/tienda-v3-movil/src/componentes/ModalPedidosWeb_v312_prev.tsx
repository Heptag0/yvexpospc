...
export default function ModalPedidosWeb({
  ...
  async function mover(p: PedidoTienda, a: EstadoPedido) {
    ...
    try {
      ...
      // v3.1: tras cancelar o marcar listo, se ofrece avisar al cliente por
      // WhatsApp (solo si tenemos su teléfono).
      if (a === "cancelado" || a === "listo") {
        const actualizado = { ...p, estado: a };
        if (telefonoParaWhatsApp(p.cliente_telefono)) {
          setAvisoCliente({ pedido: actualizado, tipo: a });
        } else if (a === "cancelado") {
          setAviso({
            texto:
              "Pedido cancelado. No tenemos el teléfono de este cliente, así que no podemos ofrecerte avisarle por WhatsApp.",
            tipo: "info",
          });
        }
      }
  ...
  function abrirWhatsAppCliente() {
    ...
    void Linking.openURL(urlWhatsAppCliente(tel, mensaje)).catch(() => {
                  ...
                  <View style={est.avisoCliente}>
                    <View style={{ flexDirection: "row", alignItems: "center", gap: 10 }}>
                      ...
                      <Text style={est.avisoClienteTxt}>
                        {avisoCliente.tipo === "cancelado"
                          ? `Pedido ${folioCortoPedido(avisoCliente.pedido.id)} cancelado. ¿Le avisamos a ${avisoCliente.pedido.cliente_nombre} por WhatsApp?`
                          : `¡${folioCortoPedido(avisoCliente.pedido.id)} listo! ¿Le avisamos a ${avisoCliente.pedido.cliente_nombre} por WhatsApp?`}
                      </Text>
                    </View>
