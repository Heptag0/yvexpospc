          ...
          {paso === 3 && (
            <View>
              ...
              <View style={est.giroGrid}>
                {GIROS.map((g) => {
                  const activo = giro?.id === g.id;
                  return (
                    <Pressable
                      key={g.id}
                      onPress={() => setGiro(g)}
                      style={[est.giroTarjeta, activo && est.tarjetaActiva]}
                    >
                      <View
                        style={[
                          est.giroIcono,
                      ...
                      <Text style={est.giroFrase} numberOfLines={2}>{g.frase}</Text>
...
function crearEstilos(T: ReturnType<typeof construirTema>) {
  return StyleSheet.create({
    ...
    // Grid de giros (2 columnas, tarjetas con ícono)
    giroGrid: {
      flexDirection: "row",
      flexWrap: "wrap",
      gap: 12,
