...
export async function crearCliente(
  ...
  await db.runAsync(
    `INSERT INTO clientes
      (id, codigo, nombre, telefono, notas, puntos, eliminado, creado_en, actualizado_en)
     VALUES (?,?,?,?,NULL,0,0,?,?)`,
    [id, codigo, nombreLimpio, telefono?.trim() || null, ahora, ahora]
  );
  return { id, codigo, nombre: nombreLimpio, telefono: telefono?.trim() || null, notas: null, puntos: 0, creado_en: ahora };
}
...
export async function editarCliente(
  ...
  const db = await bd();
  await db.runAsync(
    `UPDATE clientes SET nombre = ?, telefono = ?, notas = ?, actualizado_en = ?
     WHERE id = ?`,
    [nombreLimpio, telefono?.trim() || null, notas?.trim() || null, ahoraISO(), id]
  );
}
...
/** Búsqueda por nombre o teléfono (o lista completa si el texto va vacío). */
export async function buscarClientes(texto: string): Promise<Cliente[]> {
  const db = await bd();
  const q = texto.trim().toLowerCase();
  if (!q) {
    return db.getAllAsync<Cliente>(
      `SELECT id, codigo, nombre, telefono, notas, puntos, creado_en
  ...
  return db.getAllAsync<Cliente>(
    `SELECT id, codigo, nombre, telefono, notas, puntos, creado_en
     FROM clientes
     WHERE eliminado = 0 AND (lower(nombre) LIKE ? OR lower(COALESCE(telefono,'')) LIKE ?)
