 ...
 *  genérica resta 0.15 (penalización de variante). */
...
/** Empareja por NOMBRE contra el catálogo. PRIMERO consulta el diccionario
 *  del negocio (aliasTicket: clave normalizada → {productoId, piezasEmpaque})
 *  con el nombre crudo y con el sugerido; si hay alias y el producto sigue
 ...
 *  Si no hay alias, prueba con el nombre crudo y con el sugerido y se queda
 *  con el mejor candidato. Umbrales:
...
export function emparejarProducto(
  ...
  if (aliasMap && aliasMap.size > 0) {
    for (const nombre of [nombreCrudo, nombreSugerido]) {
      ...
      // Alias huérfano (producto borrado): se ignora y sigue el fuzzy.
    }
  }
  ...
  if (!mejor || mejorScore < UMBRAL_SUGERIDO)
    return { producto: null, estado: null, score: mejorScore };
  return {
    producto: mejor.producto,
    estado: mejorScore >= UMBRAL_SEGURO ? "seguro" : "sugerido",
    score: mejorScore,
  };
}
