...
// Toda la lógica pesada (empareje, umbrales, precio sugerido, empaques) vive
// en src/base/escaner.ts y la persistencia de la config en escanerConfig.ts;
// este archivo es solo la pantalla.
...
export default function ModalEscanearTicket({ onCerrar, onAplicado }: Props) {
  ...
  function datosLinea(le: LineaEstado): DatosLinea {
    ...
    return { prod, nEmp, cantidad, piezas, costoEmpaque, costoPieza, aviso, sugerido };
  }
  ...
  /** Recalcula precio y % desde el costo vigente (tras tocar empaque/costo). */
  ...
  function confirmarEmpareje(i: number) {
    actualizarLinea(i, { estado: "seguro", aprendido: true });
  }
   ...
   *  recalcula aviso/decisión/precio contra ESE producto. */
  ...
  async function analizar() {
    ...
    try {
      const [resp, prods, cfgEscaner, aliasMap, perfiles, cats] =
        await Promise.all([
          analizarTicket(fotoB64, fotoMime),
          listarProductos(),
      ...
      resp.lineas.forEach((linea, i) => {
        ...
        // REGLA INQUEBRANTABLE: "irreal" arranca en MANTENER; "alerta" queda
        // sin decidir (bloquea el aplicar hasta que el usuario elija).
        ...
        // Departamento sugerido por el perfil aprendido (solo productos nuevos).
        ...
        iniciales.push({
          linea,
          incluida: linea.confianza !== "baja",
          nombre: nombreBonito(linea.nombre_crudo, linea.nombre_sugerido),
  ...
  function tarjetaLinea(le: LineaEstado, i: number) {
    ...
    return (
      <View key={i} style={[est.tarjeta, atenuada && { opacity: 0.45 }]}>
        ...
        {/* Departamento (solo cuando se creará un producto nuevo) */}
        {!d.prod && (
          <View style={est.deptoCaja}>
            <Text style={est.campoLabel}>Departamento</Text>
            <View style={est.chipsWrap}>
              <Pressable
                onPress={() => actualizarLinea(i, { departamentoId: null })}
                style={[
                  est.chip,
                  {
                    borderColor: le.departamentoId == null ? T.acento : T.borde,
                    backgroundColor:
                      le.departamentoId == null ? T.acento + "1e" : T.superficie,
                  },
                ]}
              >
                <Text
                  style={[
                    ...
                    {
                      color:
                        le.departamentoId == null ? T.acento : T.textoSuave,
                    },
                  ]}
                >
                  Sin departamento
              ...
              {categorias.map((c) => {
                const activo = le.departamentoId === c.id;
                return (
                  <Pressable
                    key={c.id}
                    onPress={() => actualizarLinea(i, { departamentoId: c.id })}
                    style={[
                      est.chip,
                      {
        ...
        {le.conEmpaque ? (
          <View style={est.bloque}>
            <View style={est.filaEmpaque}>
              <View style={est.campoChico}>
                <Text style={est.campoLabel}>
                  {le.linea.unidad_ticket === "caja" ? "Cajas" : "Packs"}
                </Text>
                <TextInput
                  value={le.cantidadTxt}
                  onChangeText={(t) => actualizarLinea(i, { cantidadTxt: t })}
                  keyboardType="number-pad"
                  style={[estInput, est.inputChico]}
                />
              </View>
              <Text style={est.empaqueX}>×</Text>
              <View style={est.campoChico}>
                <Text style={est.campoLabel}>Piezas c/u</Text>
                <TextInput
              ...
              <Text style={est.empaqueIgual}>= {d.piezas} pzs</Text>
            </View>
            <Pressable onPress={() => conmutarEmpaque(i, false)} hitSlop={8}>
              <Text style={est.enlaceMini}>es pieza suelta ›</Text>
            </Pressable>
          </View>
        ) : (
          <View style={est.bloque}>
            <View style={est.filaEmpaque}>
              <View style={est.campoChico}>
                <Text style={est.campoLabel}>Piezas</Text>
                <TextInput
                  value={le.cantidadTxt}
                  onChangeText={(t) => actualizarLinea(i, { cantidadTxt: t })}
                  keyboardType="number-pad"
            ...
            <Pressable onPress={() => conmutarEmpaque(i, true)} hitSlop={8}>
              <Text style={est.enlaceMini}>¿viene en caja o pack? ›</Text>
            </Pressable>
          </View>
        )}

        {/* Costo (por empaque si viene en caja/pack; por pieza si no) */}
        <View style={[est.filaEmpaque, est.bloque]}>
          <View style={est.campoChico}>
            <Text style={est.campoLabel}>
              {le.conEmpaque ? "Costo por caja/pack" : "Costo por pieza"}
            </Text>
            <TextInput
              value={le.costoTxt}
        ...
        {/* Avisos de cambio de costo (solo contra producto existente) */}
        {d.prod && d.aviso.nivel === "ninguno" && d.aviso.pct != null && (
          <Text style={[est.datoTenue, est.bloque]}>
            El costo cambió {pctAviso} (dentro de lo normal).
          </Text>
        )}
        {d.prod && d.aviso.nivel === "aviso" && (
        ...
        {/* Decisión de costo (no aplica en "irreal": ahí manda la caja de arriba) */}
        {d.prod && d.aviso.nivel !== "irreal" && (
        ...
        {/* Precio de venta + % de ganancia (ligados) */}
        {(!d.prod || le.decision === "sugerido") && (
          <View style={[est.filaEmpaque, est.bloque]}>
            <View style={est.campoChico}>
              <Text style={est.campoLabel}>Precio de venta</Text>
              <TextInput
                value={le.precioTxt}
            ...
            <View style={est.campoChico}>
              <Text style={est.campoLabel}>% ganancia</Text>
              <TextInput
                value={le.pctTxt}
  ...
  const vistaRevision = (
    <View style={{ flex: 1 }}>
      ...
      >
        ...
        {respuesta && sumaCuadra(respuesta) === false && (
          <AvisoCaja T={T} tipo="alerta">
            Las líneas no cuadran con el total impreso en el ticket. Revisa con
            calma: la IA pudo leer mal una cantidad o un costo.
          </AvisoCaja>
        )}
        {respuesta?.avisos.map((a, idx) => (
          <AvisoCaja key={idx} T={T} tipo="info">
            {a}
          </AvisoCaja>
        ))}
        {respuesta?.mock && (
          <AvisoCaja T={T} tipo="info">
            Respuesta de prueba del servidor (mock): los datos no son de tu
            foto.
          </AvisoCaja>
        )}
        {ticketDudoso && (
          <AvisoCaja T={T} tipo="alerta">
            Este ticket se ve dudoso: la IA pudo leer mal varios costos. Revisa
            con calma; si algo no cuadra, repite la foto.
          </AvisoCaja>
        )}
...
function crearEstilos(T: Tema) {
  return StyleSheet.create({
    ...
    chipTxt: { fontSize: 12.5, fontWeight: "800" },
    bloque: { marginTop: 12 },
    filaEmpaque: {
      flexDirection: "row",
      alignItems: "flex-end",
