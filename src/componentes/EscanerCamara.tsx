...
import { sonidoBeep } from "../base/sonidos";

// TODOS los tipos que expo-camera soporta: ayuda con códigos chicos o en
// superficies curvas (botes/latas de cerveza usan a veces ITF-14, Code93…).
const TIPOS = [
  "ean13",
  "ean8",
  "upc_a",
...
export default function EscanerCamara({ modo, onCodigo, onCerrar, aviso }: Props) {
  ...
  // Zoom digital de la cámara (0 a 1): para códigos chicos o curvos se
  // acerca sin mover el teléfono. Botones − / + con pasos de 0.1.
  ...
  function alEscanear({ data }: { data: string }) {
    ...
    // Vibración + beep: que se SIENTA y se ESCUCHE que el código entró.
    // En modo continuo suena en cada lectura aceptada (post-cooldown).
          ...
          <View style={{ flex: 1 }}>
            ...
            <View style={StyleSheet.absoluteFill} pointerEvents="none">
              <View style={est.marcoZona}>
                ...
                <Text style={est.guia}>
                  Apunta al código de barras · acércalo con el zoom si está chico
                </Text>
                {modo === "continuo" && (
            ...
            {/* Control de zoom: botones − / + (pasos de 0.1, de 0 a 1).
                Sirve para códigos chicos o curvos sin acercar el teléfono. */}
            <View style={est.zoomCaja} pointerEvents="box-none">
              <Pressable
                style={[est.zoomBtn, zoom <= 0 && est.zoomBtnOff]}
                disabled={zoom <= 0}
                onPress={() => setZoom((z) => Math.max(0, Math.round((z - 0.1) * 10) / 10))}
                hitSlop={8}
              >
                <Text style={est.zoomBtnTxt}>−</Text>
              </Pressable>
              <Text style={est.zoomTxt}>zoom</Text>
              <Pressable
                style={[est.zoomBtn, zoom >= 1 && est.zoomBtnOff]}
                disabled={zoom >= 1}
                onPress={() => setZoom((z) => Math.min(1, Math.round((z + 0.1) * 10) / 10))}
                hitSlop={8}
              >
                <Text style={est.zoomBtnTxt}>+</Text>
              </Pressable>
...
const est = StyleSheet.create({
  ...
  zoomBtnTxt: { color: "#fff", fontSize: 22, fontWeight: "800", marginTop: -2 },
  zoomTxt: {
