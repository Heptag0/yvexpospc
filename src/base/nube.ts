...
import type { EntradaCatalogoIA } from "./inventario";
 ...
 *
 *  El body incluye `catalogo` (máx. 300 productos con sus alias, pack
 *  recordado y último costo): con ese contexto la IA empareja mejor y puede
 *  responder `catalogo_idx` por línea, apuntando a la entrada ENVIADA. */
export async function analizarTicket(
  imagenB64: string,
  mime: string,
  catalogo: EntradaCatalogoIA[] = []
  ...
  try {
    const res = await fetch(BASE + "/ia/ticket", {
      ...
      body: JSON.stringify({ imagen_b64: imagenB64, mime, catalogo }),
      signal: ctrl.signal,
    });
