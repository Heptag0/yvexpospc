...
// ---------------------------------------------------------------------------
// v3.1 (2ª parte) — temas SIN "auto" (el móvil deja de ofrecerlo)
// ---------------------------------------------------------------------------

assert.deepEqual(TEMAS_TIENDA, ["perla", "medianoche", "arena", "cielo"], "4 temas, sin auto");
assert.ok(!TEMAS_TIENDA.includes("auto"), "auto ya no se ofrece");
...
assert.equal(temaTiendaValido("auto"), "perla", "config vieja con auto -> perla");
assert.equal(temaTiendaValido(" AUTO "), "perla", "auto con mayúsculas y espacios");
assert.equal(temaTiendaValido(""), "perla", "vacío -> perla (default tiendas nuevas)");
assert.equal(temaTiendaValido(null), "perla", "null -> perla");
assert.equal(temaTiendaValido("sepia"), "perla", "valor raro -> perla");

assert.equal(temaTiendaEfectivo("auto", false), "perla", "auto -> perla sin importar el sistema");
assert.equal(temaTiendaEfectivo("auto", true), "perla", "auto ya no depende del sistema");
...
assert.equal(fondoGuiaTema("auto", false), "#f6f7fb", "auto -> guía de perla");
assert.equal(fondoGuiaTema("auto", true), "#f6f7fb", "auto ya no cambia con el sistema");
...
// urlCorreoCliente (v3.1: aviso por correo cuando solo hay correo).
const urlMail = urlCorreoCliente(
  "lupita@correo.com",
  "Tu pedido #A1B2C3D4 ya está listo",
  "Hola Lupita, te saluda Abarrotes Don José. Tu pedido #A1B2C3D4 ya está listo y ya puedes pasar a recogerlo."
);
assert.ok(urlMail.startsWith("mailto:lupita%40correo.com?subject="), "mailto con correo encoded");
assert.ok(urlMail.includes("&body="), "lleva body");
assert.ok(
  urlMail.includes("subject=Tu%20pedido%20%23A1B2C3D4%20ya%20est%C3%A1%20listo"),
  "asunto URL-encoded (acentos y #)"
);
assert.ok(urlMail.includes("Don%20Jos%C3%A9"), "cuerpo URL-encoded");
assert.ok(!urlMail.includes("#"), "ningún # crudo rompe la URL");
assert.equal(urlCorreoCliente("", "a", "b"), "", "correo vacío -> vacío");
assert.equal(urlCorreoCliente(null, "a", "b"), "", "null -> vacío");
assert.equal(urlCorreoCliente("   ", "a", "b"), "", "espacios -> vacío");

// ---------------------------------------------------------------------------
// v3.1 (2ª parte) — pin en el mapa (espejo de normalizar_ubicacion)
// ---------------------------------------------------------------------------

assert.equal(
  urlMapaUbicacion("25.6866,-100.3161"),
  "https://maps.google.com/?q=25.686600%2C-100.316100",
  "normaliza a 6 decimales y URL-encodea la coma"
);
assert.equal(
  urlMapaUbicacion(" 19.4326 , -99.1332 "),
  "https://maps.google.com/?q=19.432600%2C-99.133200",
  "tolera espacios alrededor de la coma"
);
assert.equal(urlMapaUbicacion(""), "", "vacía -> vacío");
assert.equal(urlMapaUbicacion(null), "", "null -> vacío");
assert.equal(urlMapaUbicacion("91,0"), "", "lat fuera de rango");
assert.equal(urlMapaUbicacion("0,181"), "", "lng fuera de rango");
assert.equal(urlMapaUbicacion("25.1234567,-100.1"), "", "más de 6 decimales no");
assert.equal(urlMapaUbicacion("hola"), "", "texto libre no");
assert.equal(urlMapaUbicacion("25.5"), "", "sin lng no");
assert.ok(urlMapaUbicacion("-90,-180").startsWith("https://maps.google.com/"), "límites exactos sí");

// ---------------------------------------------------------------------------
// v3.1 (2ª parte) — mapeo pedido web -> venta del POS
// ---------------------------------------------------------------------------

assert.equal(ORIGEN_VENTA_WEB, "web", "ventas.origen = 'web'");

assert.equal(metodoPagoVentaWeb("efectivo"), "efectivo");
assert.equal(metodoPagoVentaWeb("en_linea"), "tarjeta", "pago en línea -> bucket no-efectivo");
assert.equal(metodoPagoVentaWeb("raro"), "efectivo", "raro cae a efectivo");
assert.equal(metodoPagoVentaWeb(null), "efectivo", "null cae a efectivo");

const itemsPedido = [
  { producto_id: "p1", nombre: "Coca 600ml", cantidad: 2, precio_centavos: 1800 },
  { producto_id: "p2", nombre: "Gansito", cantidad: 1, precio_centavos: 1500 },
];
// pickup: total = suma de items, sin línea extra.
const lvPickup = lineasVentaWeb(itemsPedido, 5100, "pickup");
assert.equal(lvPickup.length, 2, "pickup sin línea de envío");
assert.equal(lvPickup[0].producto_id, "p1");
assert.equal(lvPickup[0].cantidad, 2);
// domicilio con envío (total = items + 3000): línea extra de envío.
const lvDom = lineasVentaWeb(itemsPedido, 8100, "domicilio");
assert.equal(lvDom.length, 3, "domicilio agrega la línea de envío");
assert.deepEqual(
  lvDom[2],
  { producto_id: null, nombre: "Envío a domicilio", cantidad: 1, precio_centavos: 3000 },
  "envío como línea libre por la diferencia"
);
// domicilio sin envío (total = items): no agrega línea.
assert.equal(lineasVentaWeb(itemsPedido, 5100, "domicilio").length, 2, "envío gratis no agrega línea");
// pickup con total inflado no agrega línea (la diferencia no es envío).
assert.equal(lineasVentaWeb(itemsPedido, 9999, "pickup").length, 2, "pickup ignora la diferencia");
// items vacío -> arreglo vacío (registrarVentaWeb lo rechaza cálidamente).
assert.equal(lineasVentaWeb([], 0, "pickup").length, 0, "sin items -> vacío");
...
assert.ok(
  urlPreviewTienda("https://tienda.yvexiq.com", "x", "menu", "sepia", "#fff").includes("tema=perla"),
  "tema raro cae a perla en la preview"
);
assert.ok(
  urlPreviewTienda("https://tienda.yvexiq.com", "x", "menu", "auto", "#fff").includes("tema=perla"),
  "auto canonicaliza a perla en la preview"
);

// ---------------------------------------------------------------------------
// v3.1 (2ª parte) — transiciones de pedido SIMPLIFICADAS (espejo del backend:
// nuevo → listo → entregado; cancelado desde nuevo|listo; preparando legado)
// ---------------------------------------------------------------------------

assert.ok(transicionPedidoValida("nuevo", "listo"), "nuevo -> listo (Aceptar pedido)");
assert.ok(transicionPedidoValida("nuevo", "cancelado"), "nuevo -> cancelado");
assert.ok(!transicionPedidoValida("nuevo", "preparando"), "nuevo YA NO -> preparando");
assert.ok(!transicionPedidoValida("nuevo", "entregado"), "nuevo NO -> entregado");
assert.ok(transicionPedidoValida("preparando", "listo"), "preparando legado -> listo");
assert.ok(transicionPedidoValida("preparando", "cancelado"), "preparando legado -> cancelado");
assert.ok(!transicionPedidoValida("preparando", "nuevo"), "preparando NO -> nuevo");
assert.ok(!transicionPedidoValida("preparando", "entregado"), "preparando NO -> entregado directo");
assert.ok(transicionPedidoValida("listo", "entregado"), "listo -> entregado (Marcar completado)");
